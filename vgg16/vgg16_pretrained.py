...
class VGG16_Pretrained():
    def __init__(self, sess, inputs=None):
        self.sess = sess
        self.inputs = inputs
        self.preds = self.convert_tfmodel_to_graph()
        pass
    

    
    def convert_tfmodel_to_graph(self):
        with open("./vgg16/vgg16.tfmodel", mode='rb') as f:
            fileContent = f.read()
        ...
        if self.inputs == None:
            self.inputs = tf.placeholder("float", [None, 256, 256, 3])
        tf.import_graph_def(graph_def, input_map={"images": self.inputs})
        print("graph loaded from disk")
    
        graph = self.sess.graph #tf.get_default_graph()
        prob_tensor = graph.get_tensor_by_name("import/pool4:0")
        return prob_tensor
